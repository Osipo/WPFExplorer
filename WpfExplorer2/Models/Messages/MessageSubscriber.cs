...
namespace WpfExplorer.Models
{
    public class MessageSubscriber : IDisposable
    {
        ...
        public static Guid? ExtractGuidFromVM(object reciever)
        {
            ...
            {
                r = ExtractGuidFromTypeWithGuidProp(reciever);
            }
            return r;
        }

        private static Guid? ExtractGuidFromTypeWithGuidProp(object reciever)
        {
            if (reciever == null)
                return null;

            var prop = reciever.GetType().GetProperty("Guid", System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance);
            var r = prop.GetValue(reciever);
            if (r is Guid)
                return (r as Guid?);
            return null;
        }
